...
***************************************/
    ...
    static const uint8 CYCODE USBCOMM_MIDI_SIZE[] = {
...
***************************************/
        ...
        * USBMIDI_MIDI_EP_Init() function.*/
        volatile uint16 USBCOMM_midiInPointer;                            /* Input endpoint buffer pointer */
    #else
        volatile uint8 USBCOMM_midiInPointer;                             /* Input endpoint buffer pointer */
    ...
    volatile uint8 USBCOMM_midi_in_ep;
    ...
    uint8 USBCOMM_midiInBuffer[USBCOMM_MIDI_IN_BUFF_SIZE];       /* Input endpoint buffer */
    ...
    volatile uint8 USBCOMM_midi_out_ep;                                   /* Output endpoint number */
    ...
    uint8 USBCOMM_midiOutBuffer[USBCOMM_MIDI_OUT_BUFF_SIZE];     /* Output endpoint buffer */
#endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE > 0) */

#if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF)

    static USBCOMM_MIDI_RX_STATUS USBCOMM_MIDI1_Event;            /* MIDI RX status structure */
    static volatile uint8 USBCOMM_MIDI1_TxRunStat;                         /* MIDI Output running status */
    ...
    volatile uint8 USBCOMM_MIDI1_InqFlags;                                 /* Device inquiry flag */

    #if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF)
        static USBCOMM_MIDI_RX_STATUS USBCOMM_MIDI2_Event;        /* MIDI RX status structure */
        static volatile uint8 USBCOMM_MIDI2_TxRunStat;                     /* MIDI Output running status */
        /** See description of \ref USBCOMM_MIDI1_InqFlags*/
        volatile uint8 USBCOMM_MIDI2_InqFlags;                             /* Device inquiry flag */
...
#if (USBCOMM_ENABLE_MIDI_API != 0u)
/*******************************************************************************
* Function Name: USBCOMM_MIDI_Init
****************************************************************************//**
*
...
* \globalvars
*
*  \ref USBCOMM_midiInBuffer: This buffer is used for saving and combining
*    the received data from UART(s) and(or) generated internally by
*    PutUsbMidiIn() function messages. USBCOMM_MIDI_IN_EP_Service()
*    function transfers the data from this buffer to the PC.
*
*  \ref USBCOMM_midiOutBuffer: This buffer is used by the
*    USBCOMM_MIDI_OUT_Service() function for saving the received
*    from the PC data, then the data are parsed and transferred to UART(s)
*    buffer and to the internal processing by the
*
*  \ref USBCOMM_callbackLocalMidiEvent function.
*
*  \ref USBCOMM_midi_out_ep: Used as an OUT endpoint number.
*
*  \ref USBCOMM_midi_in_ep: Used as an IN endpoint number.
*
*   \ref USBCOMM_midiInPointer: Initialized to zero.
*
* \sideeffect
...
void USBCOMM_MIDI_Init(void) 
{
   ...
   USBCOMM_midiInPointer = 0u;
        ...
        USBCOMM_LoadInEP(USBCOMM_midi_in_ep, USBCOMM_midiInBuffer,
                                                               USBCOMM_MIDI_IN_BUFF_SIZE);
    #endif  /* (USBCOMM_MIDI_IN_BUFF_SIZE > 0) */

    #if (USBCOMM_MIDI_OUT_BUFF_SIZE > 0)
        /* Provide buffer for OUT endpoint. */
        (void)USBCOMM_ReadOutEP(USBCOMM_midi_out_ep, USBCOMM_midiOutBuffer,
                                                                       USBCOMM_MIDI_OUT_BUFF_SIZE);
    #endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE > 0) */
#endif /* (USBCOMM_EP_MANAGEMENT_DMA_AUTO) */

#if (USBCOMM_MIDI_OUT_BUFF_SIZE > 0)
    USBCOMM_EnableOutEP(USBCOMM_midi_out_ep);
#endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE > 0) */

    /* Initialize the MIDI port(s) */
#if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF)
    USBCOMM_MIDI_InitInterface();
#endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF) */
}
...
#if (USBCOMM_MIDI_OUT_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI_OUT_Service
    ****************************************************************************//**
    *
    ...
    * \globalvars
    *
    *  \ref USBCOMM_midiOutBuffer: Used as temporary buffer between USB
    ...
    *******************************************************************************/
    ...
    {
    #if (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256)
        uint16 outLength;
        uint16 outPointer;
    #else
        uint8 outLength;
        uint8 outPointer;
    #endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256) */

        /* Service the USB MIDI output endpoint. */
        if (USBCOMM_OUT_BUFFER_FULL == USBCOMM_GetEPState(USBCOMM_midi_out_ep))
        {
        #if (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256)
            outLength = USBCOMM_GetEPCount(USBCOMM_midi_out_ep);
        #else
            outLength = (uint8)USBCOMM_GetEPCount(USBCOMM_midi_out_ep);
        #endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256) */

        #if (!USBCOMM_EP_MANAGEMENT_DMA_AUTO)
            #if (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256)
                outLength = USBCOMM_ReadOutEP(USBCOMM_midi_out_ep,
                                                       USBCOMM_midiOutBuffer, outLength);
            #else
                outLength = (uint8)USBCOMM_ReadOutEP(USBCOMM_midi_out_ep,
                                                              USBCOMM_midiOutBuffer, (uint16) outLength);
            #endif /* (USBCOMM_MIDI_OUT_BUFF_SIZE >= 256) */

            #if (USBCOMM_EP_MANAGEMENT_DMA_MANUAL)
                /* Wait until DMA complete transferring data from OUT endpoint buffer. */
                while (USBCOMM_OUT_BUFFER_FULL == USBCOMM_GetEPState(USBCOMM_midi_out_ep))
                {
                }
            ...
            if (outLength >= USBCOMM_EVENT_LENGTH)
            {
                ...
                #if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF)
                    if ((USBCOMM_midiOutBuffer[outPointer] & USBCOMM_CABLE_MASK) ==
                         USBCOMM_MIDI_CABLE_00)
                    {
                        USBCOMM_MIDI1_ProcessUsbOut(&USBCOMM_midiOutBuffer[outPointer]);
                    }
                    else if ((USBCOMM_midiOutBuffer[outPointer] & USBCOMM_CABLE_MASK) ==
                             USBCOMM_MIDI_CABLE_01)
                    {
                    #if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF)
                         USBCOMM_MIDI2_ProcessUsbOut(&USBCOMM_midiOutBuffer[outPointer]);
                    #endif /*  USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF */
                    }
                    else
                    {
                        ...
                        #ifdef USBCOMM_MIDI_OUT_EP_SERVICE_CALLBACK
                            USBCOMM_MIDI_OUT_EP_Service_Callback();
                        #endif /* USBCOMM_MIDI_OUT_EP_SERVICE_CALLBACK */
                    }
                #endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF) */

                    /* Process any local MIDI output functions */
                    USBCOMM_callbackLocalMidiEvent(USBCOMM_midiOutBuffer[outPointer] & USBCOMM_CABLE_MASK,
                                                            &USBCOMM_midiOutBuffer[outPointer + USBCOMM_EVENT_BYTE1]);
                    outPointer += USBCOMM_EVENT_LENGTH;
                }
            }
...
#if (USBCOMM_MIDI_IN_BUFF_SIZE > 0)
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI_IN_EP_Service
    ****************************************************************************//**
    *
    ...
    *  USBCOMM_MIDI_IN_Service() and from the
    *  USBCOMM_PutUsbMidiIn() function.
    *
    * \globalvars
    *  USBCOMM_midi_in_ep: Used as an IN endpoint number.
    *  USBCOMM_midiInBuffer: Function loads the data from this buffer to
    *    the USB IN endpoint.
    *   USBCOMM_midiInPointer: Cleared to zero when data are sent.
    *
    * \reentrant
    *  No
    *
    *******************************************************************************/
    void USBCOMM_MIDI_IN_EP_Service(void) 
    {
        ...
        if (USBCOMM_midiInPointer != 0u)
        {
            if(USBCOMM_GetEPState(USBCOMM_midi_in_ep) == USBCOMM_EVENT_PENDING)
            {
            #if (USBCOMM_EP_MANAGEMENT_DMA_AUTO)
                USBCOMM_LoadInEP(USBCOMM_midi_in_ep, NULL, (uint16)USBCOMM_midiInPointer);
            #else
                USBCOMM_LoadInEP(USBCOMM_midi_in_ep, USBCOMM_midiInBuffer,
                                                              (uint16) USBCOMM_midiInPointer);
            #endif /* (USBCOMM_EP_MANAGEMENT_DMA_AUTO) */
    ...
    *******************************************************************************/
    void USBCOMM_MIDI_IN_Service(void) 
    {
        ...
        */
        ...
        if (0u == USBCOMM_midiInPointer)
        {
            do
            {
                if (USBCOMM_midiInPointer <= (USBCOMM_MIDI_IN_BUFF_SIZE - USBCOMM_EVENT_LENGTH))
                {
                    /* Check MIDI1 input port for a complete event */
                    m1 = USBCOMM_MIDI1_GetEvent();
                    if (m1 != 0u)
                    {
                        USBCOMM_PrepareInBuffer(m1, (uint8 *)&USBCOMM_MIDI1_Event.msgBuff[0],
                                                                       USBCOMM_MIDI1_Event.size, USBCOMM_MIDI_CABLE_00);
                    }
                }
                ...
                if (USBCOMM_midiInPointer <= (USBCOMM_MIDI_IN_BUFF_SIZE - USBCOMM_EVENT_LENGTH))
                {
                    ...
                    m2 = USBCOMM_MIDI2_GetEvent();
                    if (m2 != 0u)
                    {
                        USBCOMM_PrepareInBuffer(m2, (uint8 *)&USBCOMM_MIDI2_Event.msgBuff[0],
                                                                       USBCOMM_MIDI2_Event.size, USBCOMM_MIDI_CABLE_01);
                    }
                }
            #endif /*  USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF */
            }
            while((USBCOMM_midiInPointer <= (USBCOMM_MIDI_IN_BUFF_SIZE - USBCOMM_EVENT_LENGTH)) &&
                  ((m1 != 0u) || (m2 != 0u)));
        }
    #endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF) */

        /* Service the USB MIDI input endpoint */
        USBCOMM_MIDI_IN_EP_Service();
    }


    /***************************************************************************
    * Function Name: USBCOMM_PutUsbMidiIn
    ************************************************************************//**
    *
    ...
    *  The USBCOMM_MIDI_IN_Service() function should also be called to
    ...
    *   ----------------------|-----------------------------------------
    *  USBCOMM_TRUE  | Host is not ready to receive this message
    *  USBCOMM_FALSE | Success transfer
    *
    * \globalvars
    *
    *  \ref USBCOMM_midi_in_ep: MIDI IN endpoint number used for
    *        sending data.
    *
    *  \ref USBCOMM_midiInPointer: Checked this variable to see if
    ...
    ***************************************************************************/
                                                                ...
                                                                
    {
        ...
        if (USBCOMM_midiInPointer >
                    (USBCOMM_EP[USBCOMM_midi_in_ep].bufferSize - USBCOMM_EVENT_LENGTH))
        {
            USBCOMM_MIDI_IN_EP_Service();
        }
        ...
        if (USBCOMM_midiInPointer <=
                    (USBCOMM_EP[USBCOMM_midi_in_ep].bufferSize - USBCOMM_EVENT_LENGTH))
        {
            if((ic < USBCOMM_EVENT_LENGTH) || (ic >= USBCOMM_MIDI_STATUS_MASK))
            {
                USBCOMM_PrepareInBuffer(ic, midiMsg, ic, cable);
            }
            else /* Only SysEx message is greater than 4 bytes */
            {
                ...
                do
                {
                    ...
                    USBCOMM_PrepareInBuffer(USBCOMM_MIDI_SYSEX, &midiMsg[idx],
                                                     USBCOMM_EVENT_BYTE3, cable);

                    /* Move to next 3 bytes of message */
                    ic  -= USBCOMM_EVENT_BYTE3;
                    idx += USBCOMM_EVENT_BYTE3;
                        ...
                        (USBCOMM_EP[USBCOMM_midi_in_ep].bufferSize - USBCOMM_EVENT_LENGTH))
                    {
                           ...
                           (USBCOMM_EP[USBCOMM_midi_in_ep].bufferSize - USBCOMM_EVENT_LENGTH))
                        {
                            ...
                            retError = USBCOMM_TRUE;
                            break;
                        }
                    }
                }
                while (ic >= USBCOMM_EVENT_BYTE3);

                if (retError == USBCOMM_FALSE)
                {
                    /* Handle end of message: valid size of messages is 0, 1 and 2 */
                    USBCOMM_PrepareInBuffer(USBCOMM_MIDI_EOSEX, &midiMsg[idx], ic, cable);
                }
            }
        }
        else
    ...
    /*******************************************************************************
    * Function Name: USBCOMM_PrepareInBuffer
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMM_midiInBuffer: This buffer is used for saving and combine the
    *    received from UART(s) and(or) generated internally by
    *    USBCOMM_PutUsbMidiIn() function messages.
    *  USBCOMM_midiInPointer: Used as an index for midiInBuffer to
    *     write data.
    *
    * \reentrant
    *  No
    ...
    *******************************************************************************/
                                                                 ...
                                                                 
    {
        ...
        if (ic >= (USBCOMM_MIDI_STATUS_MASK | USBCOMM_MIDI_SINGLE_BYTE_MASK))
        {
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_SINGLE_BYTE | cable;
            USBCOMM_midiInPointer++;
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = ic;
            USBCOMM_midiInPointer++;
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = 0u;
            USBCOMM_midiInPointer++;
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = 0u;
            USBCOMM_midiInPointer++;
        }
        else if((ic < USBCOMM_EVENT_LENGTH) || (ic == USBCOMM_MIDI_SYSEX))
        {
            if(ic == USBCOMM_MIDI_SYSEX)
            {
                USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_SYSEX | cable;
                USBCOMM_midiInPointer++;
            }
            else if (srcBuffZero < USBCOMM_MIDI_SYSEX)
            {
                USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = (srcBuffZero >> 4u) | cable;
                USBCOMM_midiInPointer++;
            }
            else if (srcBuffZero == USBCOMM_MIDI_TUNEREQ)
            {
                USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_1BYTE_COMMON | cable;
                USBCOMM_midiInPointer++;
            }
            else if ((srcBuffZero == USBCOMM_MIDI_QFM) || (srcBuffZero == USBCOMM_MIDI_SONGSEL))
            {
                USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_2BYTE_COMMON | cable;
                USBCOMM_midiInPointer++;
            }
            else if (srcBuffZero == USBCOMM_MIDI_SPP)
            {
                USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_3BYTE_COMMON | cable;
                USBCOMM_midiInPointer++;
            }
            else
            {
            ...
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuffZero;
            USBCOMM_midiInPointer++;
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuffOne;
            USBCOMM_midiInPointer++;
            USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuff[2u];
            USBCOMM_midiInPointer++;
        }
        else if (ic == USBCOMM_MIDI_EOSEX)
        {
            switch (eventLen)
            {
                case 0u:
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_SYSEX_ENDS_WITH1 | cable;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_MIDI_EOSEX;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = 0u;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = 0u;
                    ...
                    break;
                case 1u:
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_SYSEX_ENDS_WITH2 | cable;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuffZero;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_MIDI_EOSEX;
                    ...
                    break;
                case 2u:
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = USBCOMM_SYSEX_ENDS_WITH3 | cable;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuffZero;
                    USBCOMM_midiInPointer++;
                    USBCOMM_midiInBuffer[USBCOMM_midiInPointer] = srcBuffOne;
...
*/
#if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI_InitInterface
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *   USBCOMM_MIDI_Event: initialized to zero.
    *   USBCOMM_MIDI_TxRunStat: initialized to zero.
    *
    *******************************************************************************/
    void USBCOMM_MIDI_InitInterface(void) 
    {
        USBCOMM_MIDI1_Event.length  = 0u;
        USBCOMM_MIDI1_Event.count   = 0u;
        USBCOMM_MIDI1_Event.size    = 0u;
        USBCOMM_MIDI1_Event.runstat = 0u;
        USBCOMM_MIDI1_TxRunStat     = 0u;
        USBCOMM_MIDI1_InqFlags      = 0u;

        /* Start UART block */
        MIDI1_UART_Start();

        /* Change the priority of the UART TX and RX interrupt */
        CyIntSetPriority(MIDI1_UART_TX_VECT_NUM, USBCOMM_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI1_UART_RX_VECT_NUM, USBCOMM_CUSTOM_UART_RX_PRIOR_NUM);

    #if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF)
        USBCOMM_MIDI2_Event.length  = 0u;
        USBCOMM_MIDI2_Event.count   = 0u;
        USBCOMM_MIDI2_Event.size    = 0u;
        USBCOMM_MIDI2_Event.runstat = 0u;
        USBCOMM_MIDI2_TxRunStat     = 0u;
        USBCOMM_MIDI2_InqFlags      = 0u;

        /* Start second UART block */
        MIDI2_UART_Start();

        /* Change the priority of the UART TX interrupt */
        CyIntSetPriority(MIDI2_UART_TX_VECT_NUM, USBCOMM_CUSTOM_UART_TX_PRIOR_NUM);
        CyIntSetPriority(MIDI2_UART_RX_VECT_NUM, USBCOMM_CUSTOM_UART_RX_PRIOR_NUM);
    #endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF) */
    ...
    #ifdef USBCOMM_MIDI_INIT_CALLBACK
        USBCOMM_MIDI_Init_Callback();
    #endif /* (USBCOMM_MIDI_INIT_CALLBACK) */
    }


    /*******************************************************************************
    * Function Name: USBCOMM_ProcessMidiIn
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
                                                                ...
                                                                
    {
        ...
        */
        if ((mData & USBCOMM_MIDI_STATUS_BYTE_MASK) != 0u)
        {
            if ((mData & USBCOMM_MIDI_STATUS_MASK) == USBCOMM_MIDI_STATUS_MASK)
            {
                if ((mData & USBCOMM_MIDI_SINGLE_BYTE_MASK) != 0u) /* System Real-Time Messages(single byte) */
                {
                    midiReturn = mData;
                }
                else                              /* System Common Messages */
                {
                    switch (mData)
                    {
                        case USBCOMM_MIDI_SYSEX:
                            rxStat->msgBuff[0u] = USBCOMM_MIDI_SYSEX;
                            rxStat->runstat = USBCOMM_MIDI_SYSEX;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBCOMM_MIDI_EOSEX:
                            rxStat->runstat = 0u;
                            rxStat->size = rxStat->count;
                            rxStat->count = 0u;
                            midiReturn = USBCOMM_MIDI_EOSEX;
                            break;
                        case USBCOMM_MIDI_SPP:
                            rxStat->msgBuff[0u] = USBCOMM_MIDI_SPP;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 3u;
                            break;
                        case USBCOMM_MIDI_SONGSEL:
                            rxStat->msgBuff[0u] = USBCOMM_MIDI_SONGSEL;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBCOMM_MIDI_QFM:
                            rxStat->msgBuff[0u] = USBCOMM_MIDI_QFM;
                            rxStat->runstat = 0u;
                            rxStat->count = 1u;
                            rxStat->length = 2u;
                            break;
                        case USBCOMM_MIDI_TUNEREQ:
                            rxStat->msgBuff[0u] = USBCOMM_MIDI_TUNEREQ;
                            rxStat->runstat = 0u;
                            rxStat->size = 1u;
                            rxStat->count = 0u;
            ...
            else /* Channel Messages */
            {
                ...
                rxStat->count = 1u;
                switch (mData & USBCOMM_MIDI_STATUS_MASK)
                {
                    case USBCOMM_MIDI_NOTE_OFF:
                    case USBCOMM_MIDI_NOTE_ON:
                    case USBCOMM_MIDI_POLY_KEY_PRESSURE:
                    case USBCOMM_MIDI_CONTROL_CHANGE:
                    case USBCOMM_MIDI_PITCH_BEND_CHANGE:
                        rxStat->length = 3u;
                        break;
                    case USBCOMM_MIDI_PROGRAM_CHANGE:
                    case USBCOMM_MIDI_CHANNEL_PRESSURE:
                        rxStat->length = 2u;
                        break;
                    default:
                        rxStat->runstat = 0u;
        ...
        {
            if (rxStat->runstat == USBCOMM_MIDI_SYSEX)
            {
            ...
            else if (rxStat->runstat != 0u)
            {
                ...
                rxStat->count = 2u;
                switch (rxStat->runstat & USBCOMM_MIDI_STATUS_MASK)
                {
                    case USBCOMM_MIDI_NOTE_OFF:
                    case USBCOMM_MIDI_NOTE_ON:
                    case USBCOMM_MIDI_POLY_KEY_PRESSURE:
    ...
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI1_GetEvent
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMM_MIDI1_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /*  ((MIDI1_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBCOMM_ProcessMidiIn(rxData,
                                                    (USBCOMM_MIDI_RX_STATUS *)&USBCOMM_MIDI1_Event);
    ...
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI1_ProcessUsbOut
    ****************************************************************************//**
    *
    ...
    *
    * \globalvars
    *  USBCOMM_MIDI1_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBCOMM_MIDI1_InqFlags: The following flags are set when SysEx
    *    message comes.
    *    USBCOMM_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *    USBCOMM_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    ...
    *******************************************************************************/
    ...
    #ifdef USBCOMM_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBCOMM_MIDI1_ProcessUsbOut_EntryCallback();
    #endif /* (USBCOMM_MIDI1_PROCESS_USB_OUT_ENTRY_CALLBACK) */

        cmd = epBuf[USBCOMM_EVENT_BYTE0] & USBCOMM_CIN_MASK;

        if ((cmd != USBCOMM_RESERVED0) && (cmd != USBCOMM_RESERVED1))
        {
            len = USBCOMM_MIDI_SIZE[cmd];
            i = USBCOMM_EVENT_BYTE1;
            /* Universal System Exclusive message parsing */
            if (cmd == USBCOMM_SYSEX)
            {
                if ((epBuf[USBCOMM_EVENT_BYTE1] == USBCOMM_MIDI_SYSEX) &&
                    (epBuf[USBCOMM_EVENT_BYTE2] == USBCOMM_MIDI_SYSEX_NON_REAL_TIME))
                {
                    /* Non-Real Time SySEx starts */
                    USBCOMM_MIDI1_InqFlags |= USBCOMM_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBCOMM_MIDI1_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
                }
            }
            else if (cmd == USBCOMM_SYSEX_ENDS_WITH1)
            {
                USBCOMM_MIDI1_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBCOMM_SYSEX_ENDS_WITH2)
            {
                USBCOMM_MIDI1_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
            }
            else if (cmd == USBCOMM_SYSEX_ENDS_WITH3)
            {
                /* Identify Request support */
                if ((USBCOMM_MIDI1_InqFlags & USBCOMM_INQ_SYSEX_FLAG) != 0u)
                {
                    USBCOMM_MIDI1_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
                    if ((epBuf[USBCOMM_EVENT_BYTE1] == USBCOMM_MIDI_SYSEX_GEN_INFORMATION) &&
                        (epBuf[USBCOMM_EVENT_BYTE2] == USBCOMM_MIDI_SYSEX_IDENTITY_REQ))
                    {
                        /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBCOMM_MIDI1_InqFlags |= USBCOMM_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            ...
            if ((cmd >= USBCOMM_NOTE_OFF) && (cmd <= USBCOMM_PITCH_BEND_CHANGE))
            {
                if (USBCOMM_MIDI1_TxRunStat == epBuf[USBCOMM_EVENT_BYTE1])
                {
                ...
                else
                {
                    ...
                    USBCOMM_MIDI1_TxRunStat = epBuf[USBCOMM_EVENT_BYTE1];
                }
            }
            else
            {
    ...
    #ifdef USBCOMM_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK
        USBCOMM_MIDI1_ProcessUsbOut_ExitCallback();
    #endif /* (USBCOMM_MIDI1_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }


#if (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF)
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI2_GetEvent
    ****************************************************************************//**
    *
    *  Checks for incoming MIDI data, calls the MIDI event builder if so.
    *  Returns either empty or with a complete event.
    ...
    *
    * \globalvars
    *  USBCOMM_MIDI2_Event: RX status structure used to parse received
    *    data.
    *
    *******************************************************************************/
            ...
            */
            ...
            #endif /* ((MIDI2_UART_RXBUFFERSIZE >= 256u) && (CY_PSOC3)) */
                {
                    ...
                    */
                    ...
                    msgRtn = USBCOMM_ProcessMidiIn(rxData,
                                                    (USBCOMM_MIDI_RX_STATUS *)&USBCOMM_MIDI2_Event);
    ...
    /*******************************************************************************
    * Function Name: USBCOMM_MIDI2_ProcessUsbOut
    ****************************************************************************//**
    *
    *  Process a USB MIDI output event.
    *  Puts data into the MIDI TX output buffer.
    ...
    *
    * \globalvars
    *  USBCOMM_MIDI2_TxRunStat: This variable used to save the MIDI
    *    status byte and skip to send the repeated status byte in subsequent event.
    *  USBCOMM_MIDI2_InqFlags: The following flags are set when SysEx
    *    message comes.
    *  USBCOMM_INQ_SYSEX_FLAG: Non-Real Time SySEx message received.
    *  USBCOMM_INQ_IDENTITY_REQ_FLAG: Identity Request received.
    *    This bit should be cleared by user when Identity Reply message generated.
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
    ...
    #ifdef USBCOMM_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK
        USBCOMM_MIDI2_ProcessUsbOut_EntryCallback();
    #endif /* (USBCOMM_MIDI2_PROCESS_USB_OUT_ENTRY_CALLBACK) */
        ...
        if ((cmd != USBCOMM_RESERVED0) && (cmd != USBCOMM_RESERVED1))
        {
            ...
            if(cmd == USBCOMM_SYSEX)
            {
                ...
                {
                    /* SySEx starts */
                    USBCOMM_MIDI2_InqFlags |= USBCOMM_INQ_SYSEX_FLAG;
                }
                else
                {
                    USBCOMM_MIDI2_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
                }
            }
            else if(cmd == USBCOMM_SYSEX_ENDS_WITH1)
            {
            ...
            {
                /* Identify Request support */
                if ((USBCOMM_MIDI2_InqFlags & USBCOMM_INQ_SYSEX_FLAG) != 0u)
                {
                    USBCOMM_MIDI2_InqFlags &= (uint8)~USBCOMM_INQ_SYSEX_FLAG;
                    ...
                    {   /* Set the flag about received the Identity Request.
                        *  The Identity Reply message may be send by user code.
                        */
                        USBCOMM_MIDI2_InqFlags |= USBCOMM_INQ_IDENTITY_REQ_FLAG;
                    }
                }
            }
            else /* Do nothing for other command */
            ...
            if ((cmd >= USBCOMM_NOTE_OFF) && ( cmd <= USBCOMM_PITCH_BEND_CHANGE))
            {
                if (USBCOMM_MIDI2_TxRunStat == epBuf[USBCOMM_EVENT_BYTE1])
                {   /* Skip the repeated Status byte */
                    i++;
                }
                else
                {   /* Save Status byte for next event */
                    USBCOMM_MIDI2_TxRunStat = epBuf[USBCOMM_EVENT_BYTE1];
                }
            }
            else
            {   /* Clear Running Status */
    ...
    #ifdef USBCOMM_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK
        USBCOMM_MIDI2_ProcessUsbOut_ExitCallback();
    #endif /* (USBCOMM_MIDI2_PROCESS_USB_OUT_EXIT_CALLBACK) */
    }
#endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_TWO_EXT_INTRF) */
#endif /* (USBCOMM_MIDI_EXT_MODE >= USBCOMM_ONE_EXT_INTRF) */
...
#endif  /* defined(USBCOMM_ENABLE_MIDI_STREAMING) */
