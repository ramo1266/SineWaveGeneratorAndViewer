/*******************************************************************************
* File Name: WaveFormReader_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_Start
********************************************************************************
*
* Summary:
...
void WaveFormReader_theACLK_Start(void) 
{
    ...
    WaveFormReader_theACLK_CLKEN |= WaveFormReader_theACLK_CLKEN_MASK;
	WaveFormReader_theACLK_CLKSTBY |= WaveFormReader_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_Stop
********************************************************************************
*
* Summary:
...
void WaveFormReader_theACLK_Stop(void) 
{
    ...
    WaveFormReader_theACLK_CLKEN &= (uint8)(~WaveFormReader_theACLK_CLKEN_MASK);
	WaveFormReader_theACLK_CLKSTBY &= (uint8)(~WaveFormReader_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((WaveFormReader_theACLK_CLKEN & WaveFormReader_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = WaveFormReader_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = WaveFormReader_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* WaveFormReader_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(WaveFormReader_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(WaveFormReader_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = WaveFormReader_theACLK_GetSourceRegister();
    uint16 oldDivider = WaveFormReader_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = WaveFormReader_theACLK_CLKEN & WaveFormReader_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(WaveFormReader_theACLK_DIV_PTR, clkDivider);
                WaveFormReader_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                WaveFormReader_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(WaveFormReader_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((WaveFormReader_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(WaveFormReader_theACLK_DIV_PTR, clkDivider);
				WaveFormReader_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(WaveFormReader_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveFormReader_theACLK_MOD_SRC |= modeBitMask & (uint8)WaveFormReader_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    WaveFormReader_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(WaveFormReader_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return WaveFormReader_theACLK_MOD_SRC & (uint8)(WaveFormReader_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = WaveFormReader_theACLK_GetDividerRegister();
    uint8 oldSrc = WaveFormReader_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        WaveFormReader_theACLK_MOD_SRC |= CYCLK_SSS;
        WaveFormReader_theACLK_MOD_SRC =
            (WaveFormReader_theACLK_MOD_SRC & (uint8)(~WaveFormReader_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return WaveFormReader_theACLK_MOD_SRC & WaveFormReader_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: WaveFormReader_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    WaveFormReader_theACLK_PHASE = clkPhase & WaveFormReader_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: WaveFormReader_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return WaveFormReader_theACLK_PHASE & WaveFormReader_theACLK_PHASE_MASK;
}
